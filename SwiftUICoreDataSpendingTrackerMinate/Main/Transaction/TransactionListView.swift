...
//
//  Created by Tina Tung on 2/2/23.
//
...
struct TransactionsListView: View {
    ...
    @State var shouldAddTransactionForm = false
    ...
    var body: some View {
        VStack {
            if (fetchRequest.wrappedValue.isEmpty) {
                Text("Get started by adding your first transactions")
                
                  Button {
                      shouldAddTransactionForm.toggle()
                  } label: {
                      Text("+ Transaction")
        ...
        }.fullScreenCover(isPresented: $shouldAddTransactionForm) {
                ...
                VStack(alignment: .trailing) {
                    Button {
                        ...
                        .actionSheet(isPresented: $shouldPresentActionSheet) {
                            .init(title: Text(transaction.name ?? ""), message: nil, buttons: [.destructive(Text("Delete"), action: {
                                handleDelete()
                            }),
                            .cancel()])
                        }
